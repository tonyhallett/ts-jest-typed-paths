...
describe("transformer", () => {
  ...
  function addMockTransformToPathDependency(){
    ...
    fs.writeFileSync(path.join(transformToPathPackagePath, "package.json"), JSON.stringify(transformToPathPackageJson, null, 2));
  ...
  describe("ts-jest", () => {
    ...
    describe("transformer", () => {
      ...
      function transformTestExpected(toTransformCode:string, expectedTransformedCode:string){
        ...
        const expectedPath = tempDependentProject.createFile(expectedTransformedCode, "expectedTransformed.ts");
