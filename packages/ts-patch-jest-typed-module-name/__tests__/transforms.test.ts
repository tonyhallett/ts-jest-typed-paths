...
describe("transformer", () => {
  ...
  const packagesInstallOrder = ["transform-to-path", "jest-transform-to-path", "ts-patch-jest-typed-module-name"];
  beforeAll(() => {
...
aFn(transformToPath<typeof import("./exporting")>());`
    ...
    function tsPatchTest(code:string):{transpiled:string, processOut:string}  {
      ...
      const buffer = spawnSync(command, {shell:true, cwd: tempDependentProject.testDirectory, encoding: "utf-8"});
      const processOut = buffer.stdout.toString();

      const transpiledPath = path.join(tempDependentProject.testDirectory,"tspatchout","toTransform.js");
      ...
      function writeTsConfig(){
        ...
        return tempDependentProject.createFile(JSON.stringify(tsPatchTsConfig), "tsconfig.tspatch.json");
      }
    }
  })
});
